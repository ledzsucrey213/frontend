...
function ChapterDetails() {
  ...
  const handleSubmitEdit = async () => {
    try {
      const response = await axios.put(`http://localhost:3000/api/questions/${currentQuestion._id}`, newQuestion);
      setQuestions((prev) =>
        prev.map((question) =>
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:3000/api/questions/${currentQuestion._id}`);
      setQuestions((prev) =>
